...
def load_data(path="../data/cora/", dataset="cora"):  # path=os.path.join(sys.path[0],"../data/cora/")
    ...
    features = sp.csr_matrix(idx_features_labels[:, 1:-1], dtype=np.float32)  # 1:-1去掉第一列id和最后一列类别，csr_matrix行压缩矩阵
    labels = encode_onehot(idx_features_labels[:, -1])  # [:, -1]最后一列为每个元素的类标, 得到one-hot形式的[2708, 7]
    ...
    # 将cites中文章的编号映射到.content中的真实行, [5429, 2]
    ...
    # 邻接矩阵加上自连接并求得 \tilde{D}^{-\frac{1}{2}} \tilde{A} \tilde{D}^{-\frac{1}{2}}
...
def normalize(mx):
    """Row-normalize sparse matrix, 相当于左乘以求D^{-1}"""
    # 对于邻接矩阵，相当于求 \hat{D}
